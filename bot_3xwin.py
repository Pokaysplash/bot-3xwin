...
# 📁 Ruta del archivo JSON de Google
GOOGLE_SHEETS_CREDENTIALS = 'bot-3xwin-fb5a928147e0.json'
    ...
    try:
        ...
        if user_input in codigos:
            await context.bot.send_message(
                chat_id=chat_id,
                text=
                "✅ ¡Código válido! Aquí está tu enlace: https://t.me/Fases_3xWinBot"
            )
        else:
            await context.bot.send_message(
                chat_id=chat_id,
                text=
                "❌ No encontramos tu código. Por favor regístrate aquí: https://www.3xwin.top/formulario.html"
            )
...
# Función principal
async def main():
    # Arranca servidor keep_alive para Replit 24/7
    keep_alive()
